...
class TokenEmbeddingsHandler:
    ...
    def initialize_new_tokens(self, inserting_toks: List[str]):
        ...
        # tokens_dict = {"additional_tokens": self.inserting_toks}
        # self.tokenizer.add_tokens(tokens_dict) ## add_tokens() instead of add_special_tokens()
        self.tokenizer.add_tokens(self.inserting_toks)
        # Resize the token embeddings as we are adding new special tokens to the tokenizer
        self.text_encoder.resize_token_embeddings(len(self.tokenizer)) ## padding for optimal tensor core utilization 🤔

        # Convert the token abstractions to ids
        self.train_ids = self.tokenizer.convert_tokens_to_ids(self.inserting_toks)


        # random initialization of new tokens
        embeds = (
            self.text_encoder.embed_tokens ## cross check this
        )
        std_token_embedding = embeds.weight.data.std()

        logger.info(f"text encoder's std_token_embedding: {std_token_embedding}")

        train_ids = self.train_ids
        # if initializer_concept are not provided, token embeddings are initialized randomly
        # if args.initializer_concept is None:
        #     hidden_size = (
        #         self.text_encoder.encoder.config.hidden_size ## change here
        #     )
        ...
        self.embeddings_settings[f"original_embeddings"] = embeds.weight.data.clone()
        self.embeddings_settings[f"std_token_embedding"] = std_token_embedding
        ...
        self.embeddings_settings[f"index_no_updates"] = index_no_updates

        logger.info(self.embeddings_settings[f"index_no_updates"].shape)
    ...
    def save_embeddings(self, file_path: str):
        ...
        idx_to_text_encoder_name = {0: "qwen_vl"}
        idx = 0
        # for idx, text_encoder in enumerate(self.text_encoders):
        train_ids = self.train_ids 
        embeds = self.text_encoder.embed_tokens ## change this
        assert embeds.weight.data.shape[0] == len(self.tokenizer), "Tokenizers should be the same."
    ...
    def retract_embeddings(self):
        # for idx, text_encoder in enumerate(self.text_encoders):
        embeds = self.text_encoder.embed_tokens
        index_no_updates = self.embeddings_settings["index_no_updates"]
        embeds.weight.data[index_no_updates] = (
            self.embeddings_settings["original_embeddings"][index_no_updates]
        ...
        std_token_embedding = self.embeddings_settings["std_token_embedding"]
...
def main(args):
    ...
    kwargs = DistributedDataParallelKwargs(find_unused_parameters=True) ## for DDP
    ...
    if args.pivotal_tuning["pivotal_tuning"]:
        text_lora_parameters = []  # CLIP
        for name, param in text_encoder.named_parameters():
            # print(name)
            # import pdb; pdb.set_trace()
            if "embed_tokens" in name: ## need to change this
                # ensure that dtype is float32, even if rest of the model that isn't trained is loaded in fp16
                # import pdb; pdb.set_trace()
                # if args.train.mixed_precision == "fp16":
    ...
    # Optimizer creation
    trainable_params = list(filter(lambda p: p.requires_grad, model.parameters())) ## add text_encoder here
    # import pdb; pdb.set_trace()
    if args.pivotal_tuning["pivotal_tuning"]:
        trainable_params.extend(text_lora_parameters)
