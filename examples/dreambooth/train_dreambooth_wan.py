...
from transformers import CLIPTextModelWithProjection, CLIPTokenizer, PretrainedConfig, T5EncoderModel, T5TokenizerFast, UMT5EncoderModel
...
# AcceleratorState().deepspeed_plugin.deepspeed_config['train_micro_batch_size_per_gpu']
    ...
    if args.with_prior_preservation:
        ...
        if cur_class_images < args.num_class_images:
            ...
            for example in tqdm(
                ...
                image = pipeline(prompt=example["prompt"], num_frames=1).frames[0][0]
    ...
    def save_model_hook(models, weights, output_dir):
        if accelerator.is_main_process:
            for i, model in enumerate(models):
                if isinstance(unwrap_model(model), WanTransformer3DModel):
                    unwrap_model(model).save_pretrained(os.path.join(output_dir, "transformer"))
                elif isinstance(unwrap_model(model), UMT5EncoderModel):
    ...
    def load_model_hook(models, input_dir):
        for _ in range(len(models)):
            ...
            if isinstance(unwrap_model(model), WanTransformer3DModel):
                ...
                try:
                    load_model = UMT5EncoderModel.from_pretrained(input_dir, subfolder="text_encoder")
    ...
    if not train_dataset.custom_instance_prompts:
        ...
        # batch prompts on all training steps
            ...
            tokens_one = tokenize_prompt(tokenizer, args.instance_prompt)
            if args.with_prior_preservation:
                class_tokens_one = tokenize_prompt(tokenizer, args.class_prompt)
            ...
            with accelerator.accumulate(models_to_accumulate):
                ...
                if train_dataset.custom_instance_prompts:
                    if not args.train_text_encoder:
                        ...
                        tokens_one = tokenize_prompt(tokenizer, prompts)
    ...
    if accelerator.is_main_process:
        ...
        pipeline = WanPipeline.from_pretrained( ## here
            args.output_dir,
            revision=args.revision,
            variant=args.variant,
            torch_dtype=weight_dtype,
