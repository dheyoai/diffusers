...
def log_validation(
    ...
    with autocast_ctx:
        images = [pipeline(**pipeline_args, generator=generator).frames[0][0] for _ in range(args.num_validation_images)]
...
def main(args):
    ...
    ## need to change the LoRA layers that are compatible with WAN 2.1
    ...
    if args.lora_blocks is not None:
        ...
        target_modules = [
            f"blocks.{block}.{module}" for block in target_blocks for module in target_modules
        ]
    ...
    def load_model_hook(models, input_dir):
        ...
        if not accelerator.distributed_type == DistributedType.DEEPSPEED:
            ...
            if args.train_text_encoder:
                text_encoder_one_ = text_encoder_cls.from_pretrained(
    ...
    def get_sigmas_2(timesteps, n_dim=4, dtype=torch.float32, device='cpu', flow_shift=3.0, num_train_timesteps=1000):
        ...
        step_indices = torch.clamp(timesteps, 0, num_train_timesteps - 1).long()
        sigma = sigmas[step_indices].flatten()
        
        # Expand dimensions to match model_input
        while len(sigma.shape) < n_dim:
            sigma = sigma.unsqueeze(-1)
    ...
    def get_timesteps(u, noise_scheduler, num_train_timesteps=1000, device='cpu'):
        # Ensure u is in [0, 1] and convert to indices
        u = torch.clamp(u, 0.0, 1.0)  # Prevent numerical errors
        indices = (u * (num_train_timesteps - 1)).long()  # Map to [0, num_train_timesteps-1]
        
        # Get scheduler timesteps (ensure they are set for inference)
        timesteps = noise_scheduler.timesteps.to(device=device)
        
        # If timesteps is a subset (e.g., for inference), map indices to the closest available timestep
        max_index = len(timesteps) - 1
        indices = torch.clamp(indices, 0, max_index)
        selected_timesteps = timesteps[indices]
        ...
        for step, batch in enumerate(train_dataloader):
            ...
            with accelerator.accumulate(models_to_accumulate):
                ...
                if accelerator.sync_gradients:
                    params_to_clip = (
                        itertools.chain(
                            transformer_lora_parameters, text_lora_parameters_one
                        )
                        if args.train_text_encoder
