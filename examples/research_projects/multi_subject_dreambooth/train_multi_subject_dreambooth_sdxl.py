...
def generate_validation_images(
    text_encoder_one: Module,
    text_encoder_two: Module,
    tokenizer_one: Module,
    tokenizer_two: Module,
    unet: Module,
    vae: Module,
    ...
    if text_encoder_one is not None:
        pipeline_args["text_encoder_one"] = accelerator.unwrap_model(text_encoder_one)

    if text_encoder_two is not None:
        pipeline_args["text_encoder_two"] = accelerator.unwrap_model(text_encoder_two)
...
def parse_args(input_args=None):
    ...
    parser.add_argument(
        ...
        type=str,
        default=None,
        help="Pretrained tokenizer 2 name or path if not the same as model_name",
    )
    parser.add_argument(
        "--instance_data_dir",
        type=str,
    ...
    parser.add_argument(
        ...
        default=1,
        help=(
            "Number of time to repeat a data point in the training set"
        ),
    )
...
def main(args):
    ...
    if args.tokenizer_name:
        tokenizer_one = AutoTokenizer.from_pretrained(args.tokenizer_name)
    else:
        tokenizer_one = AutoTokenizer.from_pretrained(args.pretrained_model_name_or_path, subfolder="tokenizer")

    if args.tokenizer_2_name:
        tokenizer_two = AutoTokenizer.from_pretrained(args.tokenizer_2_name)
    else:
        tokenizer_two = AutoTokenizer.from_pretrained(args.pretrained_model_name_or_path, subfolder="tokenizer_2")
        ...
        for step, batch in enumerate(train_dataloader):
            ...
            with accelerator.accumulate(unet):
                ...
                if args.train_text_encoder:
                    ...
                    # elems_to_repeat_text_embeds = bsz // 2 if args.with_prior_preservation else bsz ## ig this is only possible if I add HF dataset support for instance prompts 😒
                    elems_to_repeat_text_embeds = 1
                    unet_added_conditions.update(
                        {"text_embeds": pooled_prompt_embeds.repeat(elems_to_repeat_text_embeds, 1)}
                    )
                    # prompt_embeds_input = prompt_embeds.repeat(elems_to_repeat_text_embeds, 1, 1)
                    prompt_embeds_input = prompt_embeds.repeat(1, elems_to_repeat_text_embeds, 1)
                    ## what is the shape of noisy_latents = torch.Size([4, 4, 128, 128]) ?
                    # import pdb; pdb.set_trace()
                    model_pred = unet(
                        noisy_latents, 
                        time_steps, 
